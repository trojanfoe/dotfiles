...
for d in $HOME/bin /opt/EtsyStore/bin $HOME/.local/bin $HOME/.cargo/bin $HOME/.docker/bin $HOME/.local/go/bin /usr/local/go/bin /opt/homebrew/opt/postgresql@17/bin
do
  if test -d $d; then
    PATH=$d:$PATH
  fi
